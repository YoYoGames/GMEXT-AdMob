...
After setting the consent mode we need to add some code to the ${event.social}, to check for the events generated by the ${function.AdMob_Consent_RequestInfoUpdate} function call.
...
Given below is the fully documented template code that you can use in your ${event.social}:
...
The first step we should take towards adding banners is to use a manager object with the following ${event.create} (remember that you need to initialize the API first - ${function.AdMob_Initialize})
...
The second step requires us to make sure the API was initialized, so for this example we will use the ASYNC SOCIAL EVENT to check for the events generated by the ${function.AdMob_Initialize} function call.
...
The first step we should take towards adding interstitials is to use a manager object with the following CREATE EVENT (remember that you need to initialize the API first - ${function.AdMob_Initialize})
...
The first step we should take towards adding rewarded videos is to use a manager object with the following CREATE EVENT (remember that you need to initialize the API first - ${function.AdMob_Initialize})
...
The second step requires us to make sure the API was initialized. For this example we will use the ASYNC SOCIAL EVENT, to check for the events generated by the ${function.AdMob_Initialize} function call.
...
The first step we should take towards adding rewarded interstitial is to use a manager object with the following CREATE EVENT (remember that you need to initialize the API first - ${function.AdMob_Initialize})
